...
def get_github_repo_commit(user_id):
    ...
    res = []
    repo_list = send_request(user_id)
    if (type(repo_list) == dict and repo_list['message']) or repo_list == []:
    ...
    for repo in repo_list:
        repo_name = repo['name']
        #print(repo_name)
        commit_list = get_repo_commit(user_id, repo_name)
...
def get_repo_commit(user_id, repo_name):
    ...
    try:
        ...
        print('Invalid request')
        return -1
    return json.loads(json_commit.text)
